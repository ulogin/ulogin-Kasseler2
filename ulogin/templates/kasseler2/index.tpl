...
<span class="headicons">$topbaner</span><a class="kasseler-cms" href="http://www.kasseler-cms.net/"><img src="templates/$load_tpl/images/kasseler-cms.png" alt="Kasseler CMS" title="Kasseler CMS" /></a>
</div></div></div>
...
all_blur = function(){for(i=0;i<mids.length;i++) $$(mids[i]).className='noac'}
for(i=0;i<mids.length;i++) if($$(mids[i]).className=='ac') active=mids[i];
for(i=0;i<mids.length;i++){
$$(mids[i]).onmouseover = function(){all_blur(); this.className='ac';}
$$(mids[i]).onmouseout = function(){if($$(active)) $$(active).className='ac'; if(active!=this.id) this.className='noac';}
$$(mids[i]).onmousedown = function(){active=this.id}
}
// -->
</script>
...
<td valign='top' id="maincol">$message $block_center $modules $block_down</td>
